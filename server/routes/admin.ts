...
// Database inspection data endpoint
  ...
  try {
    ...
    const tablesWithCounts = await Promise.all(
      tables.rows.map(async (table) => {
        try {
          const countResult = await db.execute(sql.raw(`SELECT COUNT(*) as count FROM public."${table.table_name}"`));
          return {
            ...table,
            row_count: countResult.rows[0]?.count || 0
          };
        } catch (error) {
          console.error(`Error getting row count for ${table.table_name}:`, error);
          return {
            ...table,
            row_count: 0
          };
...
// Discover all tables endpoint (alternative)
